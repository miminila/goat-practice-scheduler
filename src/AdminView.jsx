...
export default function AdminView() {
  ...
  async function removeBooking(slotTime, slotLabel) {
    if (!window.confirm("Remove the booking at " + slotLabel + "?")) return;
    setBusy(true);
    await adminRemove({ date: formatDate(selectedDay), slotTime });
    await refresh();
    ...
    <div style={s.body}>
      ...
      <div style={s.dayHeader}>
        ...
        <div style={s.dayActions}>
          <button style={s.refreshBtn} disabled={busy} onClick={refresh}>↻ Refresh</button>
      ...
      {isBlocked && (
        <div style={s.blockedBanner}>
          This day is blocked — students cannot book. Click "Unblock Day" to reopen.
        </div>
      )}
            ...
            <div key={slot.time} style={{ ...s.slotCard, ...(booking ? s.slotBooked : s.slotOpen) }}>
              ...
              {booking ? (
                <div style={s.slotInfo}>
                  ...
                  <button style={s.removeBtn} disabled={busy} onClick={() => removeBooking(slot.time, slot.label)}>Remove</button>
                </div>
              ) : (
                <div style={s.slotInfo}>
      ...
      <div style={s.settingsCard}>
        ...
        <div style={s.settingsRow}>
          ...
          {editingPhone ? (
            <>
              <input style={s.settingsInput} value={editPhone} onChange={e => setEditPhone(e.target.value)} placeholder="e.g. (555) 123-4567" />
...
const s = {
  ...
  refreshBtn: { padding: "6px 14px", borderRadius: 8, border: "1px solid #ccc", background: "white", cursor: "pointer", fontFamily: "sans-serif", fontSize: 13 },
